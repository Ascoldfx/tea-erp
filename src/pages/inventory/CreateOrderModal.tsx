...
export default function CreateOrderModal({ isOpen, onClose }: CreateOrderModalProps) {
    ...
    const [paymentTerms, setPaymentTerms] = useState<'prepayment' | 'postpayment'>('postpayment');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const summary = [
            `Заказ размещен у: ${contractor}`,
            `Сумма заказа: ${totalCost} ₴`,
            `Предоплата: ${prepayment} ₴`,
            `Условия: ${paymentTerms === 'prepayment' ? 'Предоплата' : `Отсрочка ${paymentDelay} дн`}`,
            `Доставка: ${deliveryMethod === 'pickup' ? 'Самовывоз' : 'Новая Почта'}`
        ].join('\n');
