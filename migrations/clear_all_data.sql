...
-- Сначала удаляем все связанные данные, которые могут блокировать удаление
DO $$
BEGIN
    -- Удаляем все связанные данные перед удалением items
    -- Это гарантирует, что не будет ошибок из-за внешних ключей
    
    -- Удаляем все остатки на складах (если еще не удалены)
    DELETE FROM stock_levels;
    
    -- Удаляем все движения материалов (если еще не удалены)
    DELETE FROM stock_movements;
    
    -- Удаляем все плановые расходы (если еще не удалены)
    DELETE FROM planned_consumption;
    
    -- Теперь удаляем материалы
    DELETE FROM items;
    
    RAISE NOTICE 'Все материалы успешно удалены';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Ошибка при удалении материалов: %', SQLERRM;
        RAISE;
END $$;
