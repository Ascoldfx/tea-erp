...
COMMENT ON COLUMN recipes.output_item_id IS 'ID готовой продукции из таблицы items. Может быть NULL, если готовая продукция еще не добавлена в базу данных. В этом случае техкарта сохраняется с NULL, а при загрузке создается временный идентификатор для отображения.';
